...
GROMACS's GPU offload (`-nb gpu -pme gpu -update gpu` in
`scripts/md/run_md.sh`) is backend-agnostic — the same run scripts work
unchanged on AMD. The only real difference is **how GROMACS gets built**:
